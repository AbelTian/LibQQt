...
QRectF getTagRect ( const ArrayRef<Ref<ResultPoint> >& resultPoints, const Ref<BitMatrix>& bitMatrix )
{
    ...
    if ( resultPoints->size() == 2 )
    {
        ...
        return QRectF ( QPointF ( xMin / matrixWidth, yMax / matrixHeight ), QPointF ( xMax / matrixWidth,
                                                                                       yMin / matrixHeight ) );
    }
...
QString QZXing::decodeImageFromFile ( const QString& imageFilePath, int maxWidth, int maxHeight,
