...
add_source_dir($$PWD/yaml-cpp-0.6.0/include)
add_sdk(yaml_cpp, $$add_target_name())
#add_sdk_header_no_postfix(yaml_cpp, $$add_target_name(), yaml_cpp)

#-------------------------------------------------
#用户工程配置
#-------------------------------------------------
#add_file($$PWD/yaml_cpp.pri)
include($${PWD}/yaml_cpp.pri)

CONFIG += continue_build
contains(CONFIG, continue_build){
    system("touch $${PWD}/yaml_cpp.cpp")
}
