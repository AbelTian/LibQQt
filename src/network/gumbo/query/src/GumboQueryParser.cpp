...
GumboQueryParser::GumboQueryParser ( std::string aInput )
{
    mInput = aInput;
    mOffset = 0;
}

GumboQueryParser::~GumboQueryParser()
{
}

GumboQuerySelector* GumboQueryParser::create ( std::string aInput )
{
    GumboQueryParser parser ( aInput );
    return parser.parseSelectorGroup();
}

GumboQuerySelector* GumboQueryParser::parseSelectorGroup()
{
    GumboQuerySelector* ret = parseSelector();
    ...
    while ( mOffset < mInput.size() )
    {
        ...
        GumboQuerySelector* sel = parseSelector();
        GumboQuerySelector* oldRet = ret;
...
GumboQuerySelector* GumboQueryParser::parseSelector()
{
    skipWhitespace();
    GumboQuerySelector* ret = parseSimpleSelectorSequence();
    ...
    while ( true )
    {
        ...
        GumboQuerySelector* oldRet = ret;
        GumboQuerySelector* sel = parseSimpleSelectorSequence();
        bool isOk = true;
...
GumboQuerySelector* GumboQueryParser::parseSimpleSelectorSequence()
{
    GumboQuerySelector* ret = NULL;
    ...
    if ( ret == NULL )
    {
        ret = new GumboQuerySelector();
    }

    return ret;
}

void GumboQueryParser::parseNth ( int& aA, int& aB )
{
...
int GumboQueryParser::parseInteger()
{
...
GumboQuerySelector* GumboQueryParser::parsePseudoclassSelector()
{
    ...
    name = GumboQueryUtil::tolower ( name );
    ...
    if ( name == "not" || name == "has" || name == "haschild" )
    {
        ...
        GumboQuerySelector* sel = parseSelectorGroup();
        ...
        GumboQuerySelector* ret = new CUnarySelector ( op, sel );
        sel->release();
        return ret;
    }
    else if ( name == "contains" || name == "containsown" )
    {
        ...
        value = GumboQueryUtil::tolower ( value );
        skipWhitespace();
              ...
              || name == "nth-last-of-type" )
    {
        ...
        return new GumboQuerySelector ( a, b, name == "nth-last-child" || name == "nth-last-of-type",
                                   name == "nth-of-type" || name == "nth-last-of-type" );
    }
    else if ( name == "first-child" )
    {
        return new GumboQuerySelector ( 0, 1, false, false );
    }
    else if ( name == "last-child" )
    {
        return new GumboQuerySelector ( 0, 1, true, false );
    }
    else if ( name == "first-of-type" )
    {
        return new GumboQuerySelector ( 0, 1, false, true );
    }
    else if ( name == "last-of-type" )
    {
        return new GumboQuerySelector ( 0, 1, true, true );
    }
    else if ( name == "only-child" )
    {
        return new GumboQuerySelector ( false );
    }
    else if ( name == "only-of-type" )
    {
        return new GumboQuerySelector ( true );
    }
    else if ( name == "empty" )
    {
        return new GumboQuerySelector ( GumboQuerySelector::EEmpty );
    }
    else
    {
...
GumboQuerySelector* GumboQueryParser::parseAttributeSelector()
{
    if ( mOffset >= mInput.size() || mInput[mOffset] != '[' )
    {
...
GumboQuerySelector* GumboQueryParser::parseClassSelector()
{
    if ( mOffset >= mInput.size() || mInput[mOffset] != '.' )
    {
...
GumboQuerySelector* GumboQueryParser::parseIDSelector()
{
    if ( mOffset >= mInput.size() || mInput[mOffset] != '#' )
    {
...
GumboQuerySelector* GumboQueryParser::parseTypeSelector()
{
    std::string tag = parseIdentifier();
    return new GumboQuerySelector ( gumbo_tag_enum ( tag.c_str() ) );
}

bool GumboQueryParser::consumeClosingParenthesis()
{
    size_t offset = mOffset;
    skipWhitespace();
...
bool GumboQueryParser::consumeParenthesis()
{
    if ( mOffset < mInput.size() && mInput[mOffset] == '(' )
    {
        mOffset++;
...
bool GumboQueryParser::skipWhitespace()
{
    size_t offset = mOffset;
...
std::string GumboQueryParser::parseString()
{
    size_t offset = mOffset;
...
std::string GumboQueryParser::parseName()
{
    size_t offset = mOffset;
    std::string ret;
...
std::string GumboQueryParser::parseIdentifier()
{
...
bool GumboQueryParser::nameChar ( char c )
{
    return nameStart ( c ) || ( c == '-' ) || ( c >= '0' && c <= '9' );
}

bool GumboQueryParser::nameStart ( char c )
{
    return ( c >= 'a' && c <= 'z' ) || ( c >= 'A' && c <= 'Z' ) || ( c == '_' ) || ( c > 127 );
}

bool GumboQueryParser::hexDigit ( char c )
{
    return ( c >= '0' && c <= '9' ) || ( c >= 'a' && c <= 'f' ) || ( c >= 'A' && c <= 'F' );
}

std::string GumboQueryParser::parseEscape()
{
...
std::string GumboQueryParser::error ( std::string message )
{
    size_t d = mOffset;
    std::string ds;
