...
QQtNfcClient::QQtNfcClient ( QObject* parent ) : QBluetoothSocket ( parent )
{
    initSocket();
}

QQtNfcClient::QQtNfcClient ( QBluetoothServiceInfo::Protocol socketType, QObject* parent ) :
...
void QQtNfcClient::sendConnectMessage()
{
    pline() << isOpen() << state();
...
int QQtNfcClient::sendDisConnectFromHost()
{
    pline() << isOpen() << state();
 ...
 * 状态函数
 */
void QQtNfcClient::socketStateChanged ( QBluetoothSocket::SocketState eSocketState )
{
    pline() << eSocketState;
 ...
 * 状态函数
 */
void QQtNfcClient::socketErrorOccured ( QBluetoothSocket::SocketError e )
{
    /*
 ...
 * 功能接口
 */
void QQtNfcClient::socketConnected()
{
 ...
 * 功能接口
 */
void QQtNfcClient::socketDisconnect()
{
    pline();
}

void QQtNfcClient::updateProgress ( qint64 bytes )
{
    Q_UNUSED ( bytes )
    //pline() << bytes;
}

void QQtNfcClient::connectToSingelHost()
{
    if ( !m_uuid.isNull() )
    {
...
void QQtNfcClient::readyReadData()
{
    QByteArray bytes;
    bytes = readAll();
    translator ( bytes );
